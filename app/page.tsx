    ...
    <>
          ...
          <div className="max-w-3xl">
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Link href="/guide" className="flex min-h-13 items-center justify-center gap-2 rounded-xl bg-sunset px-6 text-sm font-bold text-white hover:bg-orange-600">
                {t("Start Exploring")} <ArrowRight size={17} />
              </Link>
              <Link href="#top-checkpoints" className="flex min-h-13 items-center justify-center gap-2 rounded-xl border border-white/20 bg-white/10 px-6 text-sm font-bold text-white backdrop-blur hover:bg-white/15">
                {t("View Top Checkpoints")}
              </Link>
            </div>
            <div className="mt-9 grid gap-3 text-sm text-white/70 sm:grid-cols-3">
              <span className="rounded-2xl border border-white/10 bg-white/5 p-4"><strong className="block text-lg text-white">{t("8 villages")}</strong> {t("Caldera, beach and hilltop routes")}</span>
              <span className="rounded-2xl border border-white/10 bg-white/5 p-4"><strong className="block text-lg text-white">{t("Live status")}</strong> {t("Weather, wind and ferry notes")}</span>
              <span className="rounded-2xl border border-white/10 bg-white/5 p-4"><strong className="block text-lg text-white">{t("Local listings")}</strong> {t("Food, stays, rentals and staff")}</span>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="grid gap-3 rounded-3xl bg-white p-4 shadow-card sm:grid-cols-2 lg:grid-cols-7">
          {quickCategories.map(({ title, href, icon: Icon, color }) => (
            <Link key={title} href={href} className="group flex items-center gap-4 rounded-2xl p-4 transition hover:bg-sand lg:flex-col lg:items-start">
              ...
              <span className="font-display font-extrabold">{t(title)}</span>
            ...
            >
              <div className={`relative ${index === 0 ? "h-80 lg:h-full" : "h-56"}`}>
                ...
                <div className="absolute inset-x-4 bottom-4">
                  <h3 className="font-display text-xl font-extrabold text-white">{t(item.title)}</h3>
      ...
      <section id="top-checkpoints" className="bg-sand py-20">
        <div className="section-shell">
          <SectionHeading eyebrow={t("Top checkpoints to visit")} title={t("Save these Santorini stops.")} description={t("From famous viewpoints to volcanic beaches and archaeological landmarks.")} />
            ...
            {topCheckpoints.map((checkpoint) => (
              <article key={checkpoint.name} className="card-lift overflow-hidden rounded-3xl border border-aegean-900/10 bg-white">
                <div className="relative h-56">
                  <Image src={checkpoint.imageUrl} alt={t(checkpoint.imageAlt)} fill className="object-cover" />
                ...
                <div className="p-5">
                  <h3 className="font-display text-xl font-extrabold">{t(checkpoint.name)}</h3>
                  <p className="mt-2 text-sm leading-6 text-aegean-950/55">{t(checkpoint.description)}</p>
                  <p className="mt-4 flex items-center gap-2 rounded-xl bg-aegean-50 p-3 text-xs font-bold text-aegean-700"><MapPin size={15} /> {t("Best time")}: {t(checkpoint.bestTime)}</p>
                  <p className="mt-3 text-[11px] text-aegean-950/35">{t("Image")}: {checkpoint.imageCredit}</p>
                  <Link href={checkpoint.guideHref} className="mt-5 flex min-h-11 items-center justify-center gap-2 rounded-xl bg-aegean-950 px-4 text-sm font-bold text-white hover:bg-aegean-700">
                    {t("View Guide")} <ArrowRight size={16} />
                  </Link>
                </div>
              </article>
            ))}
      ...
      <section className="section-shell py-20">
        <SectionHeading eyebrow={t("Today in Santorini")} title={t("Know before you go.")} description={t("Fast island status cards for planning your beach day, ferry timing or sunset route.")} href="/live" linkLabel={t("View live updates")} />
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          {todayCards.map(({ label, value, detail, icon: Icon }) => (
            <article key={label} className="rounded-3xl border border-aegean-900/10 bg-white p-5 shadow-sm">
              ...
              <p className="mt-5 text-xs font-bold uppercase tracking-wider text-aegean-950/35">{t(label)}</p>
              <h3 className="mt-1 font-display text-xl font-extrabold">{t(value)}</h3>
              <p className="mt-1 text-xs text-aegean-950/45">{t(detail)}</p>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="section-shell pb-20">
        <div className="relative overflow-hidden rounded-[2rem] bg-aegean-950 p-8 text-white sm:p-12 lg:flex lg:items-center lg:justify-between">
          ...
          <div className="relative max-w-2xl">
            <span className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-[#79c8ff]"><Building2 size={16} /> {t("Local businesses")}</span>
            <h2 className="mt-4 font-display text-3xl font-extrabold tracking-tight sm:text-4xl">{t("Own a business in Santorini?")}</h2>
            <p className="mt-3 text-sm leading-6 text-white/60">{t("Advertise your hotel, restaurant, bar or rental company on Santorini Live.")}</p>
          </div>
          <Link href="/advertise" className="relative mt-7 flex min-h-13 items-center justify-center gap-2 rounded-xl bg-sunset px-6 text-sm font-bold text-white lg:mt-0">
            {t("Advertise With Us")} <ArrowRight size={17} />
          </Link>
        </div>
      </section>
    </>
