  ...
  return (
    <header className="sticky top-0 z-50 border-b border-aegean-900/5 bg-white/90 backdrop-blur-xl">
      <div className="section-shell flex h-18 items-center justify-between py-4">
        ...
        <nav className="hidden items-center gap-5 lg:flex">
          ...
          <details className="group relative">
            <summary className={`flex cursor-pointer list-none items-center gap-1 text-sm font-semibold transition hover:text-aegean-600 ${
              ...
              Stay <ChevronDown size={14} className="transition group-open:rotate-180" />
            </summary>
            <div className="absolute right-0 top-8 w-52 rounded-2xl border border-aegean-900/10 bg-white p-2 shadow-card">
              {accommodationLinks.map((link) => <Link key={link.href} href={link.href} className="block rounded-xl px-4 py-3 text-sm font-semibold text-aegean-950/65 hover:bg-aegean-50 hover:text-aegean-700">{link.label}</Link>)}
            </div>
          </details>
        </nav>
        ...
        <Link
          href="/submit-request"
          className="hidden rounded-full bg-aegean-950 px-5 py-2.5 text-sm font-bold text-white transition hover:bg-aegean-700 lg:block"
        >
          Submit request
        </Link>
      ...
      {open && (
        <nav className="border-t border-aegean-900/5 bg-white px-5 py-4 lg:hidden">
          <div className="mx-auto flex max-w-7xl flex-col">
            ...
            <p className="px-4 pb-1 pt-4 text-xs font-bold uppercase tracking-wider text-aegean-950/35">Accommodation</p>
            {accommodationLinks.map((link) => (
              <Link key={link.href} href={link.href} onClick={() => setOpen(false)} className={`rounded-xl px-4 py-3.5 font-semibold ${pathname.startsWith(link.href) ? "bg-aegean-50 text-aegean-700" : "text-aegean-950/70"}`}>{link.label}</Link>
            ))}
          </div>
        </nav>
      )}
